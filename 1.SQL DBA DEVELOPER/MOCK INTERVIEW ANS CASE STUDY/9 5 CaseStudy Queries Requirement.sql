...
-- REQUIREMENT : HOW TO REPORT ABOVE QUERY RESULT IN EXCEL OR MSBI OR POWER BI ?
-- STEP 1 :  DESIGN THE QUERY
-- STEP 2 :	 STORE THE QUERY INSIDE A VIEW
-- STEP 3 :	 ACCESS THE VIEW FROM EXCEL OR POWER BI OR MSBI.

CREATE VIEW VW_SALES_REPORT
AS
SELECT T.CalendarYear, T.CalendarQuarter, T.EnglishMonthName, P.EnglishProductName,  SUM(S.SalesAmount) AS TOTAL_SALES
FROM TIME_DATA  as T
INNER JOIN
SALES_DATA as S
ON
...
ON
S.PRODUCTKEY = P.PRODUCTKEY
GROUP BY T.CalendarYear, T.CalendarQuarter, T.EnglishMonthName, P.EnglishProductName
