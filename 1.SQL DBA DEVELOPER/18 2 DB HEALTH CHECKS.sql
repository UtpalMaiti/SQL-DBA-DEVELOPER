...
BACKUP DATABASE [PRODUCTDATABASE] TO DISK = 'D:\PRODUCTDATABASEBACKUP.bak'		-- if possible

USE MASTER 
GO
ALTER DATABASE [PRODUCTDATABASE] SET SINGLE_USER WITH ROLLBACK IMMEDIATE
DBCC CHECKDB('PRODUCTDATABASE', REPAIR_REBUILD)			-- THIS RECREATES THE METADATA OF THE DATABASE.
DBCC CHECKDB('PRODUCTDATABASE')							-- VERIFY THAT THERE ARE NO MORE CONSISTENCY ERRORS.
...
-- PAGE RESTORES:
RESTORE DATABASE [PRODUCTDATABASE]
PAGE = '1:230'			-- FILE_ID:PAGE_ID
FROM
DISK = 'D:\PRODUCTDATABASEBACKUP.bak'				-- LOCATION OF LATEST HEALTHY BACKUP
...
-- STEP 4:  CONSIDER TO INCREASE SIZE OF LOG FILE
			ALTER DATABASE [PRODUCTDATABASE] MODIFY FILE (NAME = 'PRODUCTDATABASE_log', SIZE = 1250 MB)   -- 25% INCREASE

-- STEP 5:	CONSIDER TO RESET THE RECOVERY MODEL
			ALTER DATABASE [PRODUCTDATABASE] SET RECOVERY SIMPLE	-- PERFORM FULL BACKUP BEFORE & AFTER THIS CHANGE.

-- STEP 6:	REBUILD LOG FILE [during maintenance hours]		: THIS STEP WILL RECREATE A NEW LOG FILE FOR THE DATABASE.
			ALTER DATABASE [PRODUCTDATABASE] SET EMERGENCY				-- NO NEW USERS ARE ALLOWED TO THE DATABASE. 
			ALTER DATABASE [PRODUCTDATABASE] REBUILD LOG ON (NAME = 'PRODUCTDATABASE_log', FILENAME ='D:\NEW_LOG.LDF')  -- recreate log
			DBCC CHECKDB ('PRODUCTDATABASE')
			ALTER DATABASE [PRODUCTDATABASE] SET MULTI_USER	--  PERFORM A FULL BACKUP, LOG BACKUP
...
ALTER DATABASE TEMPDB ADD FILE (NAME = 'TEMPDEV2', FILENAME = 'D:\TEMPDEV2.ndf', SIZE = 1000 MB) 
...
DBCC DROPCLEANBUFFERS				-- CLEANS BUFFER POOL OF THE SERVER

DBCC FREESYSTEMCACHE('ALL')			-- CLEANS PROCEDURE CACHE & BUFFER POOL [PARAMETERS, MEMORY TABLES] OF THE SERVER
